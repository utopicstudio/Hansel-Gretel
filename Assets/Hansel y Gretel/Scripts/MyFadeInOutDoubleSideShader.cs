...
public class MyFadeInOutDoubleSideShader : MonoBehaviour {
	...
	void Update () {
		if (isEnabled) {
			...
			if (isFadingIn && !isFadingOut) {
				...
				m_rend.material.SetFloat("_Transparency", Mathf.Lerp(0f,1f,lerpVar));
				if (lerpVar >= 1f) {
					isFadingIn = false;
					lerpVar = 0f;
				...
				m_rend.material.SetFloat("_Transparency", Mathf.Lerp(1f,0f,lerpVar));
				if (lerpVar >= 1f) {
					isFadingOut = false;
					lerpVar = 0f;
					if (m_fadeOutAfterFadeInOneTime) {
