...
namespace ControllerSelection 
{
    public class OVRInputModule : UnityEngine.EventSystems.PointerInputModule
    {
        ...
        [Tooltip("Cursor to be pased raycast hit data to, so it can render properly.")]
        public OVRCursor cursor = null;
...
#region PointerEventData pool
        ...
        protected MouseState GetGazePointerData()
        {
            ...
            leftData.worldSpaceRay = SelectionRay;
            leftData.scrollDelta = GetExtraScrollDelta();

            //Setup the cursor ray and click state
            if (cursor)
            {
                cursor.SetRay(SelectionRay);
                cursor.SetButtonPressed(OVRInput.Get(OVRInput.Button.PrimaryIndexTrigger));
            }
            ...
            if (ovrRaycaster) 
            {
                ...
                if (graphicRect != null)
                {
                   ...
                   // Vector3 worldPos = raycast.worldPosition;
                    ...
                    if (cursor)
                    {
                        cursor.SetRayHit(raycast.worldPosition, raycast.worldNormal);
                    }
                }
            }
