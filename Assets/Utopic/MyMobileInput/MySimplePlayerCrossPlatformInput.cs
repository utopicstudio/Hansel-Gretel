...
public class MySimplePlayerCrossPlatformInput : MonoBehaviour {
	...
	void Update () {
		...
		case Type.FirstPerson:
			movementVector = new Vector3 (CrossPlatformInputManager.GetAxis ("Horizontal"), 0f, CrossPlatformInputManager.GetAxis ("Vertical"));
		...
		case Type.Look:
			...
			movementVector = new Vector3 (CrossPlatformInputManager.GetAxis ("Mouse X"), 0f, 0f);
			player.localPosition += movementVector * speed;
			break;
		default:
			break;
		}
		if (CrossPlatformInputManager.GetButtonDown ("Jump"))// && checkGrounded())
