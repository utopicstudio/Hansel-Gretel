...
public class GvrReticlePointer : GvrBasePointer {
  ...
  //Fill amount stored, so we can change the pointer to show the progress on the update method
  private float gazeFillAmount = 0.0f;
  ...
  public override void SetGazeFillAmount(float FillAmount) { gazeFillAmount = FillAmount; }
  ...
  public void UpdateDiameters() {
    ...
    float inner_half_angle_radians = Mathf.Deg2Rad * ReticleInnerAngle * 0.5f * (1.0f - gazeFillAmount);
