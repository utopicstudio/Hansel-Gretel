	...
	public sealed class AmplifyImpostorNode : ParentNode
	{
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			...
			if( dataCollector.IsFragmentCategory )
			{
				...
				if( m_customImpostorType == CustomImpostorType.Spherical )
				{
					GenerateSphereImpostorFragment( hasSpec, hasEmission, dataCollector.IsSRP );
					functionResult = dataCollector.AddFunctions( finalHeader, m_functionBody, fragIN + "." + sphereFrames, fragIN + "." + viewPosName );
				}
				else
				{
					GenerateImpostorFragment( hasSpec, hasEmission, dataCollector.IsSRP );
		...
		private void GenerateSphereImpostorFragment( bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			m_functionBody = string.Empty;
			...
			if( withSpec )
				IOUtils.AddFunctionLine( ref m_functionBody, "float4 specularSample = tex2Dbias( _Specular, float4( frameUV.xy, 0, _AI_TextureBias) );" );
			if( withEmission )
		...
		void GenerateImpostorFragment( bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			m_functionBody = string.Empty;
